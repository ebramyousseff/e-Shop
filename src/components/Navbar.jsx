      ...
      <div className=" container mx-auto px-4 md:px-16 lg:px-24 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2 relative">
          <Link to="/cart">
            ...
            {products.length > 0 && (
              <span className="absolute top-0 text-xs w-3 left-3 bg-red-600 rounded-full text-white text-center">{products.length}</span>
            )}
            </Link>
            <button className="hidden md:block" onClick={()=>setIsModelOpen(true)}> Login | Register</button>
            <button className="block md:hidden"><CiUser/></button>
        </div>
      </div>
      <div className="flex justify-center items-center space-x-10 text-sm font-bold py-4">
